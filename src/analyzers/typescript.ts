...
export class TypeScriptAnalyzer extends BaseAnalyzer {
  ...
  async analyze(code: string): Promise<CodeFix[]> {
    ...
    // Check for missing semicolons (similar to JS)
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i].trim();
      if (line && !line.endsWith(';') && !line.endsWith('{') && !line.endsWith('}') && 
          !line.endsWith(',') && !line.startsWith('//') && !line.startsWith('/*') &&
          !line.match(/^(if|else|for|while|switch|function|const|let|var|return|break|continue|interface|type|enum|class)\s/)) {
        if (line.match(/[=+\-*/%]|\(|\[/)) {
          fixes.push(this.createFix(
            ...
            'warning',
            line + ';',
            'TypeScript statements should end with semicolons.'
          ));
        }
      }
    }
    ...
    // Check for missing type annotations (basic)
    ...
    for (const match of functionMatches) {
      ...
      if (params && !params.includes(':')) {
        fixes.push(this.createFix(
          ...
          'info',
          '',
          'TypeScript functions should have type annotations for better type safety.'
        ));
      }
    }
